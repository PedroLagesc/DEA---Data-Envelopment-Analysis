# Algoritmo baseado na documenta??o :
...
# RTS :texto ou numera??o definindo o modelo DEA a ser estimado/retornos para escala
# 0 - fdh : Free disposability hull - n?o assumi a convexidade;
...
# FRONT.IDX: Indices dos DMUs a partir das vari?veis de INPU e OUTPUT
...
mydata <- read_excel("procurement.xlsx")
...
x <- as.matrix(with(mydata, cbind(in1, in2, in3, in4)))
View(x)

x <- as.matrix(with(mydata, cbind(in1, in2, in3)))
#View(x)
...
# y2 <- as.matrix(with( tone, cbind(out1,out2,out3)))
